...
-- ==================== MOD MENU INJECTION (FIXED WITH RETRY) ====================

_G._ModMenuInjectionTries = 0
_G._ModMenuInjected = false

local function InitModMenu()
    if _G._ModMenuInjected then return true end
    if _G._ModMenuInjectionTries > 10 then return false end
    _G._ModMenuInjectionTries = _G._ModMenuInjectionTries + 1
    ...
    if not SettingPageDefine or not SettingCatalog then
        return false
    end
    if SettingPageDefine.ModMenu then
        _G._ModMenuInjected = true
        return true
    end
    ...
    SettingPageDefine.ModMenu = {
        Key = "ModMenu", loc = "ZN MOD", UIKey = "Setting_Page_Privacy",
        Category = menuStacks
    }
...
-- Retry timer: call InitModMenu every second until success
local function StartMenuRetry()
    if _G._ModMenuRetryTimer then return end
    _G._ModMenuRetryTimer = Game:SetTimer(1.0, true, function()
        if InitModMenu() then
            Game:ClearTimer(_G._ModMenuRetryTimer)
            _G._ModMenuRetryTimer = nil
        end
    end)
end
